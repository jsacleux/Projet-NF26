...
VALUES((SELECT MAX(RUN_ID) FROM TCH.T_SUIVI_RUN), 'work_to_soc_consultation.sql', NOW(), 'Running');
...
SET
    ...
    DIBT_IND = WRK.WRK_CONSULTATION.INDIC_DIABETE,
    TRET_ID = WRK.WRK_CONSULTATION.ID_TRAITEMENT,
    HOSP_IND = WRK.WRK_CONSULTATION.INDIC_HOSPI,
    EXEC_ID = (SELECT current_exec_id FROM CURRENT_EXEC_ID)
WHERE (
    WRK.WRK_CONSULTATION.ID_CONSULT = SOC.O_CONS.CONS_ID
);
