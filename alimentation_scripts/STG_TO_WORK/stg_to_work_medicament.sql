...
-- If (cd_medicament, catg_medicament, marque_fabri) is found in rpart, then we don't want to change the medc id 
MERGE WRK.WRK_MEDICAMENT AS WRKMED
USING(SELECT * FROM SOC.R_MEDC) AS RMED   
ON WRKMED.CD_MEDICAMENT = RMED.CD_MEDICAMENT AND WRKMED.CATG_MEDICAMENT = RMED.CATG_MEDICAMENT AND WRKMED.MARQUE_FABRI = RMED.MARQUE_FABRI
WHEN MATCHED THEN
UPDATE SET 
