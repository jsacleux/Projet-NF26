...
SELECT ID_TRAITEMENT, CD_MEDICAMENT, CATG_MEDICAMENT, 
    ...
    DSC_POSOLOGIE, ID_CONSULT, TS_CREATION_TRAITEMENT , ROW_NUMBER() OVER (PARTITION BY CATG_MEDICAMENT, MARQUE_FABRI ORDER BY CD_MEDICAMENT) AS MEDC_ID
    FROM STG.TRAITEMENT;
...
SELECT ROW_NUMBER() OVER (PARTITION BY FONCTION_PERSONNEL ORDER BY ID_PERSONNEL) AS PART_ID, ID_PERSONNEL, FONCTION_PERSONNEL, NOM_PERSONNEL, PRENOM_PERSONNEL, 
...
SELECT ROW_NUMBER() OVER (PARTITION BY 'Patient' ORDER BY ID_PATIENT) AS PART_ID, ID_PATIENT, 'Patient' AS FONCTION_INDIVIDU FROM STG.PATIENT;
