...
SELECT ROW_NUMBER() OVER (PARTITION BY 'Patient' ORDER BY ID_PATIENT)  AS PART_ID, ID_PATIENT, 'Patient' AS FONCTION_INDIVIDU, 
...
SELECT ROW_NUMBER() OVER (PARTITION BY FONCTION_PERSONNEL ORDER BY ID_PERSONNEL) AS PART_ID, ID_PERSONNEL, FONCTION_PERSONNEL, 
...
SELECT ROW_NUMBER() OVER (PARTITION BY CATG_MEDICAMENT, MARQUE_FABRI ORDER BY CD_MEDICAMENT) AS MEDC_ID, CD_MEDICAMENT, NOM_MEDICAMENT, CONDIT_MEDICAMENT,
...
SELECT ROW_NUMBER() OVER (PARTITION BY 'Patient' ORDER BY ID_PATIENT) AS PART_ID, ID_PATIENT, 'Patient' AS FONCTION_INDIVIDU, NUM_VOIE, DSC_VOIE, CMPL_VOIE, CD_POSTAL, VILLE, 
...
INSERT INTO WRK.WRK_RPART (PART_ID, INDIVIDU_ID, FONCTION_INDIVIDU) 
SELECT ROW_NUMBER() OVER (PARTITION BY FONCTION_PERSONNEL ORDER BY ID_PERSONNEL) AS PART_ID, ID_PERSONNEL, FONCTION_PERSONNEL FROM STG.PERSONNEL;
