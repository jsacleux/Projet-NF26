...
SELECT ROW_NUMBER() OVER (PARTITION BY ID_PERSONNEL, FONCTION_PERSONNEL) AS PART_ID, ID_PERSONNEL, FONCTION_PERSONNEL, NOM_PERSONNEL, PRENOM_PERSONNEL, 
...
SELECT ROW_NUMBER() OVER (PARTITION BY ID_PATIENT, 'Patient')  AS PART_ID, ID_PATIENT, 'Patient' AS FONCTION_INDIVIDU, 
...
SELECT ROW_NUMBER() OVER (PARTITION BY ID_PERSONNEL, FONCTION_PERSONNEL) AS PART_ID, ID_PERSONNEL, FONCTION_PERSONNEL, 
...
SELECT ROW_NUMBER() OVER (PARTITION BY CD_MEDICAMENT, CATG_MEDICAMENT, MARQUE_FABRI) AS MEDC_ID, CD_MEDICAMENT, NOM_MEDICAMENT, CONDIT_MEDICAMENT,
...
SELECT ROW_NUMBER() OVER (PARTITION BY ID_PATIENT, 'Patient') AS PART_ID, ID_PATIENT, 'Patient' AS FONCTION_INDIVIDU, NUM_VOIE, DSC_VOIE, CMPL_VOIE, CD_POSTAL, VILLE, 
...
SELECT ROW_NUMBER() OVER (PARTITION BY ID_PATIENT, 'Patient') AS PART_ID, ID_PATIENT, 'Patient' AS FONCTION_INDIVIDU FROM STG.PATIENT;

INSERT INTO WRK.WRK_RPART (PART_ID, INDIVIDU_ID, FONCTION_INDIVIDU) 
SELECT ROW_NUMBER() OVER (PARTITION BY ID_PERSONNEL, FONCTION_PERSONNEL) AS PART_ID, ID_PERSONNEL, FONCTION_PERSONNEL FROM STG.PERSONNEL;
