...
SELECT
    ID_CONSULT,
    (SELECT PART_ID FROM WRK.WRK_RPART WRKPER WHERE STGCONS.ID_PERSONNEL = WRKPER.INDIVIDU_ID AND WRKPER.FONCTION_INDIVIDU <> 'Patient'),
    (SELECT PART_ID FROM SOC.R_PART SOCPAT WHERE STGCONS.ID_PATIENT = SOCPAT.SRC_ID AND 'Patient' = SOCPAT.SRC_TYP),
    TS_DEBUT_CONSULT,
    TS_FIN_CONSULT,
    POIDS_PATIENT,
    TEMP_PATIENT,
...
UPDATE WRK.WRK_CONSULTATION WRKCONS
SET ID_PATIENT = (SELECT PART_ID FROM WRK.WRK_RPART WRKPAT WHERE (SELECT ID_PATIENT FROM STG.CONSULTATION STGCONS WHERE STGCONS.ID_CONSULT = WRKCONS.ID_CONSULT) = WRKPAT.INDIVIDU_ID AND 'Patient' = WRKPAT.FONCTION_INDIVIDU)
