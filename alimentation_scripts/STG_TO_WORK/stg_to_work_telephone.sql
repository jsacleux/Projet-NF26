...
CREATE VOLATILE TABLE MAX_PART_ID (
    max_part_id INT
) PRIMARY INDEX (max_part_id)
ON COMMIT PRESERVE ROWS;

INSERT INTO MAX_PART_ID (max_part_id)
SELECT MAX(PART_ID) FROM SOC.O_TELP;
...
SELECT
    (SELECT max_part_id FROM MAX_PART_ID) + ROW_NUMBER() OVER (PARTITION BY 'Patient' ORDER BY ID_PATIENT) AS PART_ID,
    ID_PATIENT,
